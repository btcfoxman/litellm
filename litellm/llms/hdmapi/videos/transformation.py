...
class HDMAPIVideoConfig(BaseVideoConfig):
    ...
    def _infer_provider_model(self, model: str) -> str:
        if "pro" in model:
            return "sora-2-pro"
        return "sora-2"
    ...
    def _infer_size(self, model: str) -> str:
        is_portrait = "portrait" in model
        is_hd = "hd" in model

        if is_portrait and is_hd:
            return "1024x1792"
        if (not is_portrait) and is_hd:
            return "1792x1024"
        if is_portrait:
            return "720x1280"
        return "1280x720"
    ...
    def transform_video_create_request(
        ...
        request_data: Dict[str, Any] = {
            "model": self._infer_provider_model(model),
            "prompt": prompt,
            "seconds": self._infer_seconds(model),
            "size": self._infer_size(model),
        }

        for key in ("model", "seconds", "size", "watermark", "private", "style"):
