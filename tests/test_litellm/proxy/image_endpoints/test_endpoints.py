...
@pytest.mark.asyncio
async def test_image_generation_redacts_data_url_for_pre_call_hook_but_restores_for_route(monkeypatch):
    async def fake_add_litellm_data_to_request(**kwargs):
        return kwargs["data"]
    ...
    original_data_url = "data:image/jpeg;base64,ZmFrZV9pbWFnZQ=="
    body = orjson.dumps(
        {
            "model": "gemini-3.1-flash-image-landscape",
            "prompt": "watercolor style",
            "input_reference": [original_data_url],
        }
    )
    ...
    assert result == {"result": "ok"}
    assert proxy_logger_calls["pre_call_input"]["input_reference"] == [
        "data:image/*;base64,<redacted>"
    ]
    assert captured_route_request_data["input_reference"] == [original_data_url]
